...
import { X, Plus, Edit2, Trash2, Check, AlertCircle, Loader2, Briefcase } from "lucide-react";
...
interface ManageDesignationsModalProps {
    ...
    onDesignationsChanged: (newlyCreatedName?: string) => Promise<void> | void;
}

export default function ManageDesignationsModal({
    isOpen,
    onClose,
    designations,
    onDesignationsChanged,
}: ManageDesignationsModalProps) {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                setNewName("");
                await onDesignationsChanged(trimmed);
            } else {
                setErrorMsg(data.error || "Failed to add designation");
            }
        } catch {
    ...
    const startEdit = (des: DesignationItem) => {
        setEditingId(des._id);
        setEditName(des.name);
        setErrorMsg("");
    };
    ...
    const handleSaveEdit = async (des: DesignationItem) => {
        const trimmed = editName.trim();
        if (!trimmed || trimmed === des.name) {
            cancelEdit();
            return;
        }
        setErrorMsg("");
        ...
        try {
            ...
            if (data.success) {
                setEditingId(null);
                setEditName("");
                await onDesignationsChanged();
            } else {
                setErrorMsg(data.error || "Failed to rename designation");
            }
        } catch {
            setErrorMsg("Network error");
        } finally {
    ...
    const handleDelete = async (des: DesignationItem) => {
        setErrorMsg("");
        setIsDeleting(true);
        ...
        try {
            ...
            if (data.success) {
                ...
                setErrorMsg(data.error || "Failed to delete designation");
            }
        } catch {
            setErrorMsg("Network error");
        } finally {
            ...
            >
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-black text-gray-900">Manage Staff Designations</h2>
                            <p className="text-xs text-gray-400">Add, rename, or delete designations</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <form onSubmit={handleAdd} className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                placeholder="e.g. Fleet Mechanic, Security Officer..."
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden max-h-[320px] overflow-y-auto">
                            {designations.map((des) => {
                                const isEditingThis = editingId === des._id;
                                const isConfirmingDelete = deletingId === des._id;

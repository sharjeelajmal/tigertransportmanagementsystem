...
// Creates a manual Credit (receipt from customer) or Debit (payment to outsider)
// ledger entry for a specific party.
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!body.narration?.trim()) {
            return NextResponse.json({ success: false, error: 'Narration is required' }, { status: 400 });
        }
